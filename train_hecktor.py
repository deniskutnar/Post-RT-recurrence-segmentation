...
def main():
    h5file_train = h5py.File("train_dataset.hdf5", 'r')
    h5file_val = h5py.File("val_dataset.hdf5", 'r')
    ...
    class Trainer:
        ...
        def run_trainer(self):
            early_stopping = EarlyStopping(patience=patience_rate, verbose=True, delta=1e-4)
            if self.notebook:
            ...
            for i in progressbar:
                ...
                if self.validation_DataLoader is not None:
                    self._validate()
                    early_stopping(self.valid_LL, model)
    ...
    train_dataset = HecktorDataset(h5file_train, num_samples=1, transform=transformations)
    val_dataset = HecktorDataset(h5file_val, num_samples=1, transform=None)
    
    xxx, yyy =  train_dataset[0]
    print("Image: ", xxx.shape)
    print("Mask: ", yyy.shape)
    
    dataloader_training = DataLoader(train_dataset, batch_size=2, num_workers=8, shuffle=True)
    dataloader_validation = DataLoader(val_dataset, batch_size=2, num_workers=8, shuffle=True)
    ...
    print("Final result : ", np.max(validation_dice))
    torch.save([training_losses, validation_losses, lr_rates, training_dice, validation_dice], 'trainer_stats.pt')
