...
def main():
    ...
    model.load_state_dict(torch.load('models/UNet-model_HECKTOR.pt'))
    model.cuda()
    ...
    class Trainer:
        ...
        def run_trainer(self):
            early_stopping = EarlyStoppingMetric(patience=patience_rate, verbose=True, delta=1e-4, path='models/UNet-model-finetune.pt')
            if self.notebook:
    ...
    torch.save([training_losses, validation_losses, lr_rates, training_dice, validation_dice], 'stats/trainer_stats_finetune.pt')
